...
if __name__ == '__main__':
    ...
    try:
        ...
        for item in configRoot.find("alerts").iterfind("alert"):
            ...
            alert.host = str(configRoot.find("smtp").find("server").attrib["host"])
            alert.port = int(configRoot.find("smtp").find("server").attrib["port"])
            ...
            alert.templateFile = make_path(str(item.find("mail").attrib["templateFile"]))
            ...
            if (alert.host != "127.0.0.1"
                or alert.port != 25):
                raise NotImplementedError('Only SMTP server host "127.0.0.1" and port "25" is implemented')
