...
from ..globalData.sensorObjects import SensorDataType, SensorDataInt, SensorDataFloat, SensorErrorState
...
class Prometheus:
    ...
    def update_sensor_data(self, sensor: ManagerObjSensor):
        ...
        # Set value to NaN if sensor is disconnected or has an error.
        if node.connected == 0 or sensor.error_state.state != SensorErrorState.OK:
            gauge.set(float("nan"))
        else:
