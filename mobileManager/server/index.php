...
if(isset($_GET["activate"])) {
	...
	// set the alert system active entry according to the given parameter
	if($activate === 0) {
		mysql_query("UPDATE options "
			. "SET value = 0 "
			. "WHERE type = 'alertSystemActive'");
	}
	else if($activate === 1) {
		mysql_query("UPDATE options "
			. "SET value = 1 "
		...
		<script>
			...
			function processResponse() {
				if (request.readyState == 4) {
					...
					// alertSystemInformation[0] => options
					// alertSystemInformation[1] => nodes
					// alertSystemInformation[2] => sensors
					// alertSystemInformation[3] => managers
					// alertSystemInformation[4] => alerts
					// alertSystemInformation[5] => sensorAlerts
					// alertSystemInformation[6] => alertLevels
					alertSystemInformation = JSON.parse(response);
					options = alertSystemInformation[0];
					nodes = alertSystemInformation[1];
					sensors = alertSystemInformation[2];
					managers = alertSystemInformation[3];
					alerts = alertSystemInformation[4];
					sensorAlerts = alertSystemInformation[5];
					alertLevels = alertSystemInformation[6];
