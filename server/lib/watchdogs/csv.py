...
# This class handles all changes to users CSV that can be handled at runtime.
class CSVWatchdog(threading.Thread):
    ...
    def _createHash(self, fileLocation: str) -> str:
        """
        Creates a MD5 hash of the given file.
    ...
    def _syncUsernamesAndConnections(self):
        """
        This function synchronizes the existing usernames with existing connections.
        When a connection still exists with a username which does not exist anymore, it is closed.
        """
        for serverSession in self.serverSessions:
            ...
            if not self.userBackend.userExists(username):
                self.logger.info("[%s]: Username '%s' does not exist anymore. Closing connection to client."
                                 % (self.fileName, username))
                serverSession.closeConnection()

    def _syncUsernamesAndDatabase(self):
        """
        This function synchronizes the existing usernames with the entries in the database.
        When an entry still exists for a username which does not exist anymore, delete it.
        """
        nodesList = self.storage.getNodes(self.logger)

        if nodesList is None:
            self.logger.error("[%s]: Not able to retrieve nodes from database." % self.fileName)
            return
        ...
        for nodeObj in nodesList:
            ...
            if not self.userBackend.userExists(username):
                self.logger.info("[%s]: Username '%s' does not exist anymore. Removing node from database."
                                 % (self.fileName, username))

                if not self.storage.deleteNode(nodeId, self.logger):
                    self.logger.error("[%s]: Not able to delete node with id '%d'." % (self.fileName, nodeId))
    ...
    def run(self):
        ...
        while True:
            ...
            for i in range(5):
                if self.exitFlag:
                    self.logger.info("[%s]: Exiting ConfigWatchdog." % self.fileName)
                    return
                time.sleep(1)
            ...
            if self.CSVUsersCheck:
                ...
                if diffTime > self.configCheckInterval:
                    self.logger.debug("[%s]: Checking changes of CSV users file." % self.fileName)
                    ...
                    if self.CSVUsersHash != newHash:
                        self.logger.info("[%s]: CSV users file changed." % self.fileName)
